...
| stale_after_days | int | No | Days after last_confirmed before belief is considered stale. When stale: confidence MUST be reduced by 0.1 per day overdue, minimum 0.3. Stale beliefs MUST NOT be excluded from exports — runtimes decide how to handle them |
...
3. Fetch the issuer's public key from:
   GET {issuer.url}/.well-known/engram-keys
4. Verify the Ed25519 signature against the public key
5. If valid AND expires_at is in the future: trust the context
6. If invalid OR expired: reject and log. Do not use the context.
```
...
Multiple keys may be listed to support key rotation. Runtimes MUST match `kid` from the export envelope (if present) or try all active keys.
...
```json
{
  ...
  "changes": {
    "beliefs": {
      ...
      "updated": [ ... ],
      "deleted": [
        { "id": "b_003", "status": "deleted", "deleted_at": "2026-04-20T08:00:00Z" }
      ]
    },
    "corrections": {
      "added": [ ... ]
...
- If `engram_version` major version is higher than supported: reject with `{"error": {"code": "unsupported_version", "supported": "0.1"}}`
